...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=1, in_chans=64, embed_dim=128, norm_layer=None):
        super().__init__()
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        out = x_out.view(-1, H, W, self.num_features[0]).permute(0, 3, 1, 2).contiguous()
                #if i < 1:
        outs.append(out)
        #print(len(outs))
        #print(len(outs))
...
class SECONDv1(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 pretrained=None):
        super(SECONDv1, self).__init__(init_cfg=init_cfg)
        assert len(layer_strides) == len(layer_nums)
        assert len(out_channels) == len(layer_nums)
        ...
        for i, layer_num in enumerate(layer_nums):
            ...
            if i == 1:
                block = [
                   DCN(in_filters[i], out_channels[i], kernel_size=3, stride=2, padding=1),
                   build_norm_layer(norm_cfg, out_channels[i])[1],
                   nn.ReLU(inplace=True),
                   ]
                for j in range(layer_num):
        ...
        self.conv1 = nn.Sequential(
            DCN(64, 64, kernel_size=3, stride=2, padding=1),
        ...
        self.conv2 = nn.Sequential(
            DCN(64, 128, kernel_size=3, stride=2, padding=1),
        ...
        self.conv3 = nn.Sequential(
            DCN(128, 256, kernel_size=3, stride=2, padding=1),
...
class DarkNet(nn.Module):
    ...
    def _make_layer(self, planes, blocks):
        ...
        #layers.append(("ds_conv", DCN(self.inplanes, planes[1], kernel_size=3, stride=2, padding=1)))
        #DCN(self.inplanes, planes[1], kernel_size=3, stride=2, padding=1),
    ...
    def _make_layer1(self, planes, blocks):
        ...
        # 下采样，步长为2，卷积核大小为3
        #layers.append(("ds_conv", nn.Conv2d(self.inplanes, planes[1], kernel_size=3, stride=2, padding=1, bias=False)))
        layers.append(("ds_conv", DeformableConv2d(64, planes[1], kernel_size=3, stride=2, padding=1, bias=False)))
        #DCN(self.inplanes, planes[1], kernel_size=3, stride=2, padding=1),
        layers.append(("ds_bn", nn.BatchNorm2d(planes[1])))
        layers.append(("ds_relu", nn.LeakyReLU(0.1)))
        # 加入残差结构
    ...
    def _make_layer2(self, planes, blocks):
        ...
        # 下采样，步长为2，卷积核大小为3
        #layers.append(("ds_conv", nn.Conv2d(self.inplanes, planes[1], kernel_size=3, stride=2, padding=1, bias=False)))
        layers.append(("ds_conv", DeformableConv2d(128, planes[1], kernel_size=3, stride=2, padding=1, bias=False)))
        #DCN(self.inplanes, planes[1], kernel_size=3, stride=2, padding=1),
        layers.append(("ds_bn", nn.BatchNorm2d(planes[1])))
        layers.append(("ds_relu", nn.LeakyReLU(0.1)))
        # 加入残差结构
